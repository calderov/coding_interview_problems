...
class Solution:
    def depthSum(self, nestedList: List[NestedInteger]) -> int:
        return sum([self.valueTimesDepth(node, 1) for node in nestedList])

    def valueTimesDepth(self, node, depth):
        if node.isInteger():
            return depth * node.getInteger()
        return sum(self.valueTimesDepth(innerNode, depth + 1) for innerNode in node.getList())
    
# NOTE: THIS PROBLEM HAS REQUIRES AN EXTERNAL INTERFACE IMPLEMENTED BY LEETCODE, THUS I CAN'T PUT A FULL EXAMPLE HERE