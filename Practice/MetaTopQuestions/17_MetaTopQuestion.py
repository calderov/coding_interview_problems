...
class Solution:
    # Time complexity: O(n log (n) + m) where n is the number of characters in s and m is the number of characters in order
    # Space complexity: O(n + m)
    def CustomSortString(self, order, s):
        order = {order[i]:i for i in range(len(order))}

        sortable = [c for c in s if c in order]
        remain   = [c for c in s if c not in order]

        sortable.sort(key=lambda x: order[x])

        return ''.join(sortable + remain)
