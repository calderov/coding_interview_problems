...
# Time: O(n)
# Space: O(n)
def InsertIntervalCopy(intervals, newInterval):
    if not intervals:
        return [[newInterval]]
...
# Time: O(n)
# Space: O(1)
def InsertIntervalInPlace(intervals, newInterval):
    if not intervals:
        return [[newInterval]]
    ...
    while intervals:
        ...
        if isOverlap(interval, newInterval):
            if queue and queue[-1] == newInterval:
                queue.pop()
            newInterval = merge(interval, newInterval)
    ...
    while queue:
        intervals.append(queue.popleft())

    return intervals

def InsertInterval(intervals, newInterval):
    return InsertIntervalInPlace(intervals, newInterval)
