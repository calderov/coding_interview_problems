...
# Time: O(n * m) where n = |strings| and m = len(min(strings, key=len))
# Space: O(m) where m = len(min(strings, key=len))
def LongestCommonPrefix(strings):
    shortestS = min(strings, key=len)
    
    for i in range(len(shortestS)):
        c = shortestS[i]
        for s in strings:
            if s[i] == c:
                continue
