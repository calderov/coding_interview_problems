...
def GetUp(matrix, i, j):
    ...
    # If the current position is al ready at the top, there is no up
    if i == 0:
        return None
...
def GetDown(matrix, i, j):
    ...
    # If the current position is al ready at the bottom, there is no down
    if i == rows - 1:
        return None
...
def GetRight(matrix, i, j):
    ...
    # If the current position is al ready at the rightmost position, there is no right
    if j == cols - 1:
        return None
...
def GetLeft(matrix, i, j):
    ...
    # If the current position is al ready at the leftmost position, there is no left
    if j == 0:
        return None
...
def exploreIsland(matrix, row, col):
    visited = []
    pending = [(row, col)]
    ...
    while pending:
        ...
        down = GetDown(matrix, i, j)
        if down and down not in visited:
            pending.append(down)

        left = GetLeft(matrix, i, j)
        if left and left not in visited:
            pending.append(left)

        right = GetRight(matrix, i, j)
    ...
    for i in range(rows):
        for j in range(cols):
            if matrix[i][j] and (i, j) not in visited:
                visited += exploreIsland(matrix, i, j)
                islands += 1
