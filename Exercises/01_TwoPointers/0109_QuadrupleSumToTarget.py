...
class Solution:
    ...
    def QuadrupletSumToTargetV1(self, nums, target):
        quadruplets = []
        n = len(nums)
    ...
    # Space complexity: O(n choose 4) as that is the max number of possible quadruplet combinations in nums.
    def QuadrupletSumToTargetV2(self, nums, target):
        quadruplets = []
        n = len(nums)
        nums.sort()
    ...
    def QuadrupletSumToTarget(self, nums, target):
        return self.QuadrupletSumToTargetV2(nums, target)
...
if __name__ == "__main__":
    ...
    output = solution.QuadrupletSumToTarget(nums, target)
