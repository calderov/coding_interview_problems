...
class Solution:
    ...
    def CompareStringsContainingBackspacesV1(self, string1, string2):
        return self.ApplyBackspaces(string1) == self.ApplyBackspaces(string2)

    def ApplyBackspaces(self, string):
        stack = []
        for c in string:
            if c == '#':
                if len(stack) > 0:
    ...
    def CompareStringsContainingBackspacesV2(self, string1, string2):
        i = len(string1) - 1
        j = len(string2) - 1

        while i >= 0 and j >= 0:
            if string1[i] == '#':
                i = self.GetNextValidIndex(string1, i)

            if string2[j] == '#':
                j = self.GetNextValidIndex(string2, j)
    ...
    def CompareStringsContainingBackspaces(self, string1, string2):
        return self.CompareStringsContainingBackspacesV2(string1, string2)
...
if __name__ == "__main__":
    ...
    string2 = "xzz#"
    expectedOutput = True
    output = solution.CompareStringsContainingBackspaces(string1, string2)
